...
function love.load()
	...
	for id, data in ipairs(map_nodes_data) do
		...
		if data.imagePath then
			-- This converts the text string into a picture Love2D can use
...
function love.draw()
	if gamestate == "explore" then
		-- 0. Draw the Background Image (if it exists)
		if currentNode.image then
			-- Get the width and height of the game window (800 x 600)
			...
			-- Calculate the scale (Window Size / Image Size)
			-- Example: If window is 800 and image is 1600, scale is 0.5 (50%)
			local scaleX = windowWidth / imageWidth
			local scaleY = windowHeight / imageHeight

			-- Draw the image with the calculated scale
			-- args: draw(image, x, y, rotation, scaleX, scaleY)
